"use strict";const{app:c,BrowserWindow:T,screen:w,ipcMain:u,shell:C,Tray:N,Menu:O,nativeImage:M}=require("electron"),m=require("node:path"),d=require("fs");process.platform==="linux"&&(c.commandLine.appendSwitch("enable-transparent-visuals"),c.commandLine.appendSwitch("disable-gpu-compositing"),c.disableHardwareAcceleration());let y=null,a=null;const{exec:l,spawn:$}=require("child_process");function F(){return new Promise(t=>{const e=Buffer.from(`
...
`,"utf16le").toString("base64");l(`powershell -NoProfile -EncodedCommand ${e}`,{maxBuffer:5*1024*1024},(s,r)=>{if(s||!r)return t([]);try{const o=JSON.parse(r.trim());t(Array.isArray(o)?o:o?[o]:[])}catch{t([])}})})}function W(){return new Promise(t=>{const e=Buffer.from(`
[Console]::OutputEncoding = [System.Text.Encoding]::UTF8
$results = [System.Collections.Generic.List[object]]::new()
...
@($results) | ConvertTo-Json -Compress -Depth 2
`,"utf16le").toString("base64");l(`powershell -NoProfile -EncodedCommand ${e}`,{maxBuffer:2*1024*1024},(s,r)=>{if(s||!r)return t([]);try{const o=JSON.parse(r.trim());t(Array.isArray(o)?o:o?[o]:[])}catch{t([])}})})}async function R(){const[t,n]=await Promise.all([F(),W()]),e=new Set,s=[];for(const r of[...t,...n]){if(!r.name||!(r.path||r.appId))continue;const o=r.type==="uwp"?`shell:AppsFolder\\${r.appId}`:r.path,i=o.toLowerCase();e.has(i)||(e.add(i),s.push({name:r.name,launch:o}))}return s.sort((r,o)=>r.name.localeCompare(o.name))}function b(t){const n=[];let e=0;for(;e<t.length;){for(;e<t.length&&/\s/.test(t[e]);)e++;if(e>=t.length)break;let s="";for(;e<t.length&&!/\s/.test(t[e]);)if(t[e]==='"'){for(e++;e<t.length&&t[e]!=='"';)s+=t[e++];e<t.length&&e++}else s+=t[e++];s&&n.push(s)}return n}function j(t){const n=t.replace(/\//g,"\\").replace(/%([^%]+)%/g,(o,i)=>process.env[i]||`%${i}%`),e=n.match(/^"([^"]+)"(.*)/);if(e)return{exe:e[1],args:e[2].trim()?b(e[2].trim()):[]};const s=n.match(/^(.+?\.(?:exe|cmd|bat|com|ps1))(?:\s+(.*))?$/i);if(s)return{exe:s[1],args:s[2]?b(s[2]):[]};const r=n.search(/\s/);return r===-1?{exe:n,args:[]}:{exe:n.slice(0,r),args:b(n.slice(r+1).trim())}}function I(t){const n=t.trim();if(n.startsWith("shell:")){const e=n.replace(/'/g,"''");l(`powershell -NoProfile -WindowStyle Hidden -Command "Start-Process '${e}'"`);return}if(/[\\\/]/.test(n)){const{exe:e,args:s}=j(n);if(s.length===0){if(e.toLowerCase().endsWith(".url")){try{const p=d.readFileSync(e,"utf8").match(/^URL=(.+)$/im);p&&C.openExternal(p[1].trim())}catch{}return}C.openPath(e).then(i=>{i&&l(`start "" "${e}"`)});return}const r=/[\\/]/.test(e)&&!/\.[^\\.]+$/.test(e)?e+".exe":e;if(/\.(cmd|bat)$/i.test(r)){const i=$("cmd.exe",["/c",r,...s],{shell:!1,detached:!0,stdio:"ignore"});i.on("error",()=>{}),i.unref();return}if(/\.ps1$/i.test(r)){const i=$("powershell.exe",["-NoProfile","-ExecutionPolicy","Bypass","-File",r,...s],{shell:!1,detached:!0,stdio:"ignore"});i.on("error",()=>{}),i.unref();return}const o=$(r,s,{shell:!1,detached:!0,stdio:"ignore"});o.on("error",()=>{}),o.unref();return}if(n.includes(" ")){const e=n.replace(/'/g,"''");l(`powershell -NoProfile -WindowStyle Hidden -Command "Start-Process '${e}'"`)}else l(`start "" ${n}`)}u.handle("set-ignore-mouse-events",(t,n,e)=>{a&&(process.platform!=="linux"?a.setIgnoreMouseEvents(n,{forward:e||!1}):a.setIgnoreMouseEvents(n))});u.handle("focus-window",()=>{a&&a.focus()});u.handle("open-external",async(t,n)=>{await C.openExternal(n)});u.handle("launch-app",async(t,n)=>{const e=process.platform;e==="darwin"?l(`open -a "${n}"`):e==="win32"?I(n):l(n)});u.handle("build-app-cache",async()=>{if(process.platform!=="win32")return;const t=m.join(c.getPath("userData"),"app-cache.json");try{const n=await R();d.writeFileSync(t,JSON.stringify(n))}catch{}});u.handle("search-apps",async(t,n)=>{if(process.platform!=="win32"||!n)return[];const e=m.join(c.getPath("userData"),"app-cache.json");try{if(!d.existsSync(e))return[];const s=JSON.parse(d.readFileSync(e,"utf8")),r=n.toLowerCase();return s.filter(o=>o.name&&o.name.toLowerCase().includes(r)).slice(0,8)}catch{return[]}});u.handle("get-displays",()=>w.getAllDisplays().map(n=>({id:n.id,label:n.label||`Display ${n.id}`,bounds:n.bounds})));u.handle("set-display",(t,n)=>{if(a){const s=w.getAllDisplays().find(f=>f.id.toString()===n.toString())||w.getPrimaryDisplay(),{x:r,y:o,width:i,height:p}=s.bounds;process.platform,a.setBounds({x:r,y:o,width:i,height:p}),a.show()}});u.handle("update-window-position",(t,n,e)=>{});u.handle("set-auto-launch",(t,n)=>{if(process.platform==="linux"){const e=m.join(c.getPath("home"),".config","autostart"),s=m.join(e,"ripple.desktop");try{if(n){d.existsSync(e)||d.mkdirSync(e,{recursive:!0});const r=`[Desktop Entry]
Type=Application
Version=1.0
Name=Ripple
...
Icon=${P()}
Terminal=false
`;d.writeFileSync(s,r)}else d.existsSync(s)&&d.unlinkSync(s)}catch(r){console.error("Failed to set auto-launch on Linux:",r)}}else if(process.platform==="win32")try{c.setLoginItemSettings({openAtLogin:n,path:c.getPath("exe")})}catch(e){console.error("Failed to set login item settings on Windows:",e)}});const P=()=>{const t="png";if(c.isPackaged){const n=m.join(process.resourcesPath,`icon.${t}`),e=m.join(process.resourcesPath,`assets/icons/icon.${t}`);return d.existsSync(n)?n:d.existsSync(e)?e:n}return m.join(__dirname,`../../src/assets/icons/icon.${t}`)},x=()=>{const t=w.getPrimaryDisplay(),{x:n,y:e,width:s,height:r}=t.bounds,o=process.platform==="linux",i=process.platform==="win32",p=process.platform==="darwin",f=s,h=r,g=n,S=e,k=i?"toolbar":"panel";a=new T({width:f,height:h,x:g,y:S,backgroundColor:"#00000000",transparent:!0,alwaysOnTop:!0,resizable:!1,frame:!1,...i?{}:{thickFrame:!1},hasShadow:!1,skipTaskbar:!0,icon:P(),...p?{hiddenInMissionControl:!0}:{},type:k,fullscreen:!1,visibleOnFullScreen:!0,acceptFirstMouse:!0,webPreferences:{preload:m.join(__dirname,"preload.js"),devTools:!1},show:!0}),o?a.setIgnoreMouseEvents(!0):a.setIgnoreMouseEvents(!0,{forward:!0});const E=o?500:0;a.once("ready-to-show",()=>{setTimeout(()=>{a&&(a.show(),o?a.setAlwaysOnTop(!0,"screen-saver"):a.setAlwaysOnTop(!0,"pop-up-menu"),a.focus())},E)}),setTimeout(()=>{a&&!a.isVisible()&&(a.show(),a.focus())},5e3),a.on("closed",()=>{a=null});try{a.setVisibleOnAllWorkspaces(!0,{visibleOnFullScreen:!0})}catch{}if(!c.isPackaged||process.env.NODE_ENV==="development")a.loadURL("http://localhost:5173");else{const A=m.join(__dirname,"../renderer/main_window/index.html");a.loadFile(A)}};c.whenReady().then(()=>{process.platform==="darwin"&&c.dock.hide(),x(),c.on("activate",()=>{T.getAllWindows().length===0&&x()});try{const t=P(),e=M.createFromPath(t).resize({width:16,height:16});y=new N(e);const s=O.buildFromTemplate([{label:"Show/Hide Ripple",click:()=>{a&&(a.isVisible()?a.hide():a.show())}},{type:"separator"},{label:"Quit",click:()=>{c.quit()}}]);y.setToolTip("Ripple"),y.setContextMenu(s)}catch(t){console.error("Failed to create tray:",t)}});u.handle("get-system-media",async()=>new Promise(t=>{const n=process.platform;n==="darwin"?l(`osascript -e '
