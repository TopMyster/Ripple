...
`;m.writeFileSync(n,o)}else m.existsSync(n)&&m.unlinkSync(n)}catch(o){console.error("Failed to set auto-launch on Linux:",o)}}else if(process.platform==="win32")try{l.setLoginItemSettings({openAtLogin:t,path:l.getPath("exe")})}catch(s){console.error("Failed to set login item settings on Windows:",s)}}});const S=()=>{const e="png";if(l.isPackaged){const t=f.join(process.resourcesPath,`icon.${e}`),s=f.join(process.resourcesPath,`assets/icons/icon.${e}`);return m.existsSync(t)?t:m.existsSync(s)?s:t}return f.join(__dirname,`../../src/assets/icons/icon.${e}`)},k=()=>{const e=w.getPrimaryDisplay(),{x:t,y:s,width:n,height:o}=e.bounds,r=process.platform==="linux",a=process.platform==="win32",u=process.platform==="darwin",y=n,d=o,g=t,x=s,b=a?void 0:"toolbar";i=new A({width:y,height:d,x:g,y:x,backgroundColor:"#00000000",transparent:!0,alwaysOnTop:!0,resizable:!1,frame:!1,...a?{}:{thickFrame:!1},hasShadow:!1,skipTaskbar:!0,icon:S(),...u?{hiddenInMissionControl:!0}:{},...b?{type:b}:{},fullscreen:!1,visibleOnFullScreen:!0,webPreferences:{preload:f.join(__dirname,"preload.js"),devTools:!1},show:!1}),r||i.setFullScreen(!0),i.setIgnoreMouseEvents(!0,{forward:!0});const P=r?500:0;i.once("ready-to-show",()=>{setTimeout(()=>{i&&(i.show(),r&&i.setAlwaysOnTop(!0,"screen-saver"),i.focus())},P)}),setTimeout(()=>{i&&!i.isVisible()&&(i.show(),i.focus())},5e3),i.on("closed",()=>{i=null});try{i.setVisibleOnAllWorkspaces(!0,{visibleOnFullScreen:!0})}catch{}if(!l.isPackaged||process.env.NODE_ENV==="development")i.loadURL("http://localhost:5173");else{const $=f.join(__dirname,"../renderer/main_window/index.html");i.loadFile($)}};l.whenReady().then(()=>{process.platform==="darwin"&&l.dock.hide(),k(),l.on("activate",()=>{A.getAllWindows().length===0&&k()});try{const e=S(),s=N.createFromPath(e).resize({width:16,height:16});h=new R(s);const n=D.buildFromTemplate([{label:"Show/Hide Ripple",click:()=>{i&&(i.isVisible()?i.hide():i.show())}},{type:"separator"},{label:"Quit",click:()=>{l.quit()}}]);h.setToolTip("Ripple"),h.setContextMenu(n)}catch(e){console.error("Failed to create tray:",e)}});p.handle("get-system-media",async()=>new Promise(e=>{const t=process.platform;t==="darwin"?c(`osascript -e '
            ...
            '`,(n,o)=>{if(n)return e(null);const r=o.trim();if(!r||r==="None"||r==="Error")return e(null);const a=r.split("||");a.length>=4?e({name:a[2],artist:a[3],album:a[4],artwork_url:a[5]||null,state:a[1]==="playing"?"playing":"paused",source:a[0]}):e(null)}):t==="win32"?c(`powershell -Command "${`
        ...
        return "null"
      `.replace(/"/g,'\\"')}"`,(n,o)=>{if(n||!o||o.trim()==="null"){c(`powershell "Get-Process | Where-Object {$_.ProcessName -eq 'Spotify'} | Select-Object MainWindowTitle"`,(r,a)=>{var y;if(r||!a)return e(null);const u=(y=a.split(`
`).find(d=>d.includes("-")))==null?void 0:y.trim();if(u){const[d,g]=u.split(" - ");e({name:g||u,artist:d||"Unknown",state:"playing",source:"Spotify"})}else e(null)});return}try{const r=JSON.parse(o);e({name:r.Title||"Unknown Title",artist:r.Artist||"Unknown Artist",album:r.Album||"",state:r.Status==="playing"?"playing":"paused",source:r.Source||"System"})}catch{e(null)}}):t==="linux"?c('playerctl metadata --format "{{title}}||{{artist}}||{{album}}||{{status}}"',(s,n)=>{if(s||!n)return e(null);const o=n.trim().split("||");e({name:o[0],artist:o[1],album:o[2],state:o[3].toLowerCase(),source:"System"})}):e(null)}));p.handle("get-bluetooth-status",async()=>new Promise(e=>{const t=process.platform;t==="darwin"?c("system_profiler SPBluetoothDataType -json",(s,n)=>{if(s)return e(!1);try{const r=JSON.parse(n).SPBluetoothDataType[0],a=r.device_connected&&r.device_connected.length>0;e(a)}catch{e(!1)}}):t==="win32"?c(`powershell -Command "${`
        Add-Type -AssemblyName System.Runtime.WindowsRuntime
